...
private:
    ...
    int CreateOrGetTextureFromPngData(const char* key, unsigned char* data, int width, int height);
    void ReleaseTextureByKey(const char* key); // decrefs cache entry

    bool LoadImageIfNeeded(PngImageData& imageData, const char* path, bool& flag);
...
private:
    static std::unordered_map<std::string, std::pair<int,int>> s_textureCache;
};
