...
private:
    ...
    void DrawTabIcon(int tabIndex, int tabX, int tabY, int tabWidth, int tabHeight, bool isActive);
};
...
public:
    CTabIconPanel(Panel* parent, const char* name, int texId = -1)
        : Panel(parent, name), m_texId(texId)
    {
        SetMouseInputEnabled(false);
    }

    void SetTextureId(int id) { m_texId = id; InvalidateLayout(); Repaint(); }
...
CPngPropertySheet::CPngPropertySheet(Panel* parent, const char* panelName, bool draggableTabs)
    : BaseClass(parent, panelName, draggableTabs)
{
}
...
int CPngPropertySheet::LoadPNGTexture(const char* pngPath)
{
    ...
    if (ext && (_stricmp(ext, ".vtf") == 0 || _stricmp(ext, ".VTF") == 0))
    {
		// fixme: its a vtf, not a png
		int texId = surface()->DrawGetTextureId( pngPath );
		if ( texId == -1 )
		{
			texId = surface()->CreateNewTextureID();
			surface()->DrawSetTextureFile( texId, pngPath, false, true );
		}

        m_textureCache[pngPath] = texId;
        DevMsg("CPngPropertySheet: Loaded VTF '%s' -> ID %d\n", pngPath, texId);
        return texId;
    }
...
void CPngPropertySheet::PerformLayout()
{
    ...
    struct TabCandidate { Panel* p; int x,y,w,h; };
    ...
    for (int j = 0; j < GetChildCount(); ++j)
    {
        ...
        {
            TabCandidate t; t.p = child; t.x = cx; t.y = cy; t.w = cw; t.h = ch;
    ...
    std::sort(candidates.Base(), candidates.Base() + candidates.Count(),
        [](const TabCandidate& a, const TabCandidate& b) { return a.x < b.x; });

    std::vector<int> toRemove;
    for (auto &kv : m_tabIconPanels)
    {
        if (kv.first >= numPages) toRemove.push_back(kv.first);
    }
    for (int idx : toRemove)
    {
        Panel* p = m_tabIconPanels[idx];
        if (p) { p->SetParent((Panel*)NULL); delete p; }
    ...
    for (int pageIndex = 0; pageIndex < numPages; ++pageIndex)
    {
        ...
        if (itIcon == m_tabIcons.end() || itIcon->second.textureId == -1) continue;

        if (pageIndex >= candidates.Count()) continue;
...
void CPngPropertySheet::Paint()
{
    ...
    for (int pageIndex = 0; pageIndex < numPages; ++pageIndex)
    {
        ...
        if (it == m_tabIcons.end() || it->second.textureId == -1) continue;
...
public:
    CStaticMDLPanel(vgui::Panel *parent, const char *panelName, const char *command)
        : CMDLPanel(parent, panelName)
    {
		m_command = command;
	}
...
public:
    CModelButton(Panel *parent, const char *panelName, const char *modelPath, 
                 const char *displayName, const char *command, int hoverTexId = -1)
...
public:
    CSearchPanel(Panel *parent, const char *name)
...
public:
    CImageButton(Panel *parent, const char *panelName,
             int texId, const char *displayName, const char *command,
             int hoverTexId = -1, int contenticonid = -1, int contenticonhoverid = -1)
		: Button(parent, panelName, "")
		, m_texId(texId)
		, m_hoverTexId(hoverTexId)
...
public:
	CSpawnPage(Panel *parent, const char *name, const char *spawnListName = nullptr)
		: BaseClass(parent, name)
		, m_pViewport(nullptr)
    ...
    enum EntryType { ENTRY_HEADER, ENTRY_PROP, ENTRY_TEXT, ENTRY_IMAGE };
	...
	struct SpawnEntry {
		...
		SpawnEntry()
			: type(ENTRY_TEXT), x(0), y(0), w(0), h(0), visibleByFilter(true) {}
	};
	...
	{
		if (!displayName || !modelPath) return false;

		for (const auto& e : m_entries) {
			if (e.type == ENTRY_PROP) {
				const char* entryName  = e.name.c_str();
				const char* entryModel = e.model.c_str();
				if (entryName && entryModel &&
					Q_stricmp(entryName, displayName) == 0 &&
					Q_stricmp(entryModel, modelPath) == 0) {
					return true;
				}
			}
		}
	...
	{
		if (!displayName || !imagePath) return false;

		for (const auto& e : m_entries) {
			if (e.type == ENTRY_IMAGE) {
				const char* entryName  = e.name.c_str();
				const char* entryImage = e.image.c_str();
				if (entryName && entryImage &&
					Q_stricmp(entryName, displayName) == 0 &&
					Q_stricmp(entryImage, imagePath) == 0) {
					return true;
				}
			}
		}
...
private:
        ...
        for (KeyValues *pChild = pKV->GetFirstSubKey(); pChild; pChild = pChild->GetNextKey())
        {
            ...
            else if (Q_stricmp(keyName, "prop") == 0)
            {
                ...
                if (asMdlPanel)
                {
                    ...
                    Q_snprintf(command, sizeof(command), "prop_physics_create %s",
                               strncmp(e.model.c_str(), "models/", 7) == 0 ? e.model.c_str() + 7 : e.model.c_str());
                    e.command = command;
                    m_entries.push_back(std::move(e));
                }
            }
	...
	void ComputeLayoutPositions()
	{
		...
		enum GridType { GRID_NONE = 0, GRID_PROP = 1, GRID_IMAGE = 2 };
		...
		auto flushSection = [&](void) {
			...
			int perRowH = (activeGrid == GRID_IMAGE) ? (imageBtnH + labelHeight + padding)
													: (propBtnH + labelHeight + padding);
    ...
    Panel *CreatePanelForEntry(int index)
    {
		...
		else if (e.type == ENTRY_IMAGE)
		{
			...
			CImageButton *pBtn = new CImageButton(m_pContentPanel, CFmtStr("ImageBtn_%d", index), texId, e.name.c_str(), e.command.c_str(), m_hoverTexId,
										conId, conhovId);
			pBtn->SetBounds(e.x, e.y, e.w, e.h);
			m_spawnItems.AddToTail(pBtn);
			return pBtn;
    ...
    void ClearAllEntries()
    {
        ...
        {
            if (p.second) p.second->MarkForDeletion();
        }
...
public:
	...
	{
		if (!m_pPngSheet) return false;
		if (tabIndex < 0 || tabIndex >= m_pPngSheet->GetNumPages()) return false;
...
void CSpawnMenu::CreateTabs()
{
    ...
    for (int i = 0; i < kvFiles.Count(); i++)
    {
        ...
        const char* iconPath = pTabKey->GetString("page_icon", "materials/icon16/folder_page.png");
...
static int sm_CreateTab(lua_State* L) {
    ...
    if (!g_SpawnMenu.SpawnMenu) {
        lua_pushnil(L); lua_pushstring(L, "spawn menu not created");
        return 2;
    }

    int idx = g_SpawnMenu.SpawnMenu->Lua_CreateTab(tabName, spawnListName, iconPath);
    if (idx < 0) {
        lua_pushnil(L); lua_pushstring(L, "failed to create tab");
        return 2;
    }
...
static int sm_GetTabName(lua_State* L) {
    ...
    if (!pageName) { lua_pushnil(L); return 1; }
...
static int sm_HeaderExists(lua_State* L) {
    ...
    if (!g_SpawnMenu.SpawnMenu) { lua_pushboolean(L, 0); return 1; }
...
static const luaL_Reg smlib[] = {
    {"CreateTab",         sm_CreateTab},
    {"CreateModelButton", sm_CreateModelButton},
    {"CreateImageButton", sm_CreateImageButton},
    {"CreateHeader",      sm_CreateHeader},
    {"FindTabByName",     sm_FindTabByName},
    {"TabExists",         sm_TabExists},
    {"GetNumTabs",        sm_GetNumTabs},
    {"GetTabName",        sm_GetTabName},
    {"HeaderExists",      sm_HeaderExists},
    {"ModelButtonExists", sm_ModelButtonExists},
    {"ImageButtonExists", sm_ImageButtonExists},
    {"RemoveEntry",       sm_RemoveEntry},
    {"ClearTab",          sm_ClearTab},
    {"CreateModelButtonInHeader", sm_CreateModelButtonInHeader},
    {"CreateImageButtonInHeader", sm_CreateImageButtonInHeader},
    {NULL, NULL}
};
