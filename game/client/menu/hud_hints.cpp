...
public:
    ...
    virtual void Paint() override { /* nothing to paint lol */ }
    ...
    void SetMaxHints( int n ) { m_nMaxHints = n; }
...
CHudHints::CHudHints( const char *pElementName ) 
    : CHudElement( pElementName ), Panel( NULL, "HudHints" )
{
...
void CHudHints::Think()
{
    ...
    for ( size_t idx = 0; idx < m_Notices.size(); ++idx )
    {
        ...
        if ( p->Length < 0 ) timeleft = 1.0f;

        if ( timeleft < 0.7f ) ideal_x -= 50;
        if ( timeleft < 0.2f ) ideal_x += w * 2;
        ...
        if ( fabs( dist ) < 2.0f && fabs( p->VelY ) < 0.1f ) p->VelY = 0.0f;
        dist = ideal_x - x;
        p->VelX = p->VelX + dist * spd * 1.0f;
        if ( fabs( dist ) < 2.0f && fabs( p->VelX ) < 0.1f ) p->VelX = 0.0f;
...
static const luaL_Reg hintlib[] = {
    { "AddHint", Hints_AddHint },
    { "AddNotify", Hints_AddNotify },
    { NULL, NULL }
};
