...
public:
    CColorPickerForwardPanel( vgui::Panel *parent, vgui::Panel *pForwardTo, int target )
        : vgui::Panel( parent, "ColorPickerForward" ), m_pForwardTo( pForwardTo ), m_iTarget( target ) {}

    MESSAGE_FUNC_PARAMS( OnColorPicked_Internal, "ColorPickerPicked", data )
    {
        if ( !data || !m_pForwardTo ) return;
