...
static void DrawRoundedBox(int x, int y, int w, int h, int radius, Color col)
{
    ...
    cornerCx[0] = x + radius;          cornerCy[0] = y + radius;          startAng[0] = PI;         // top-left
    cornerCx[1] = x + w - radius;      cornerCy[1] = y + radius;          startAng[1] = -PI / 2.0f; // top-right
    cornerCx[2] = x + w - radius;      cornerCy[2] = y + h - radius;      startAng[2] = 0.0f;       // bottom-right
    cornerCx[3] = x + radius;          cornerCy[3] = y + h - radius;      startAng[3] = PI / 2.0f;  // bottom-left
...
static int LoadPNGTexture(const char* filename)
{
    ...
    unsigned char* rgba = stbi_load_from_memory(
        (unsigned char*)buf.Base(), buf.TellPut(), &w, &h, &c, 4);
